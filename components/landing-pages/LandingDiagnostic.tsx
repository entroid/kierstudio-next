...
export function LandingDiagnostic({
  ...
  return (
    <section id="diagnostico" data-section="DIAGNOSTIC" className="py-24 md:py-32 bg-[#F5F5F5]">
        ...
        <div className="grid lg:grid-cols-[40%_1fr] gap-16 items-start mb-20">
          ...
          <div className="space-y-4">
            {items.map((item, i) => {
              ...
              return (
                <motion.div key={i} initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.12, duration: 0.5 }} className="group flex gap-6 p-8 bg-white border border-[#28292D]/8 hover:border-[#D52169] transition-colors duration-300">
          ...
          <div className="relative z-10">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {outcomes.items.map((item, index) => (
                <div key={index} className="flex gap-4 items-start p-8 bg-white border border-[#28292D]/8 hover:border-[#D52169] transition-colors duration-300">
