    ...
    <section id="problemas" data-section="PROBLEMS" className="py-24 md:py-32 bg-white">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="mb-16">
          <span className="font-archivo text-[0.6875rem] tracking-[0.3em] uppercase text-[#28292D]/50 mb-6 block italic" style={{ fontWeight: 600 }}>
            {tag}
          </span>
          <h2 className="font-archivo text-[2.5rem] md:text-[4rem] leading-[0.9] tracking-[-0.03em] text-[#28292D] uppercase" style={{ fontWeight: 900 }}>
            {title1}<br />
            <span className="text-[#D52169]">{titleHighlight}</span>
          </h2>

          <p className="font-archivo text-[1.2rem] text-[#28292D]/60 leading-[1.7] mt-6" style={{ fontWeight: 400 }}>
            {subTitleSection}
          </p>
        </motion.div>
          ...
          {items.map((p, i) => {
            ...
            return (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.12, duration: 0.5 }} className="group p-8 border border-[#28292D]/10 hover:border-[#D52169] transition-colors duration-300">
                ...
                <h3 className="font-archivo text-[1.125rem] text-[#28292D] mb-4 leading-[1.3]" style={{ fontWeight: 700 }}>
                  {p.title}
                </h3>
                <p className="font-archivo text-[0.9375rem] text-[#28292D]/60 leading-[1.7]" style={{ fontWeight: 400 }}>
                  {p.body}
                </p>
              </motion.div>
            );
